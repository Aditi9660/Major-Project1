...
public class PackageService extends BaseHandler {
   ...
   public PackageDTO displayPackageDetails(Long packageId){
    ...
    public TrackingDTO getByTrackingNo(String TrackingNo){
        Optional<Tracking> track = trackingRepository.findByTrackingNo(TrackingNo);
         if(track.isPresent()){
             Tracking entity = track.get();
             return new TrackingDTO(entity.getTrackingNo(), entity.getDeliveryStatus(), entity.getDeliveryDate());
         }
         else{
             return null;
         }
